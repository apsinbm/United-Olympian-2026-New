      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row gap-16">
          ...
          <div className="lg:w-1/3">
            <span className="text-gold font-bold tracking-widest uppercase mb-2 block">
              {t("actionPlan.sectionLabel")}
            </span>
            <h2 className="text-4xl md:text-5xl font-extrabold mb-6 leading-tight">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-gold to-yellow-200">
                {t("actionPlan.sectionTitleHighlight")}
              </span>
            </h2>
          ...
          <div className="lg:w-2/3 space-y-4">
            {actionPlanKeys.map((key, index) => {
              ...
              const hiddenOnMobile =
                !showAllMobile && index >= MOBILE_INITIAL_COUNT;
              return (
                <div
                  key={key}
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-4 text-left">
                      <span
                        className={`flex items-center justify-center w-8 h-8 rounded-full font-bold text-sm ${isOpen ? "bg-navy-deep text-gold" : "bg-navy-deep/50 text-gray-400"}`}
                      >
                        {index + 1}
                      </span>
                      <h3
                        className={`font-bold text-lg md:text-xl ${isOpen ? "text-navy-deep" : "text-gray-100"}`}
                      >
                        {t(`actionPlan.items.${key}.title`)}
                      </h3>
                    </div>
                    {isOpen ? (
                      <ChevronUp className="shrink-0 ml-4 text-crimson" />
                    ) : (
                      <ChevronDown className="shrink-0 ml-4 text-gray-400" />
                    )}
                  </button>
            ...
            {!showAllMobile && (
              <div className="md:hidden text-center pt-4">
                ...
                >
                  {t("actionPlan.viewAll").replace(
                    "{count}",
                    String(actionPlanKeys.length),
                  )}
